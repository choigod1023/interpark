...
def close_1():
    # 사용자 압력값 유효성검사
    if (str(e1.text()) == "") | (str(e2.text()) == "") | (str(e3.text()) == "") | (len(str(e3.text())) < 6) | (str(e4.text()) == "") | (str(e6.text()) == "회차") | (str(e7.text()) == "") | ((cb1.isChecked() | cb2.isChecked() | cb3.isChecked() | cb4.isChecked() | cb5.isChecked() |cb6.isChecked()|cb7.isChecked())== False):
        if str(e1.text()) == "" :
            msg("ID를 입력하십시오.")
            e1.setFocus()
        elif str(e2.text()) == "" :
...
# 전역변수 선언 - 함수 호출을 위해서
    ...
    global cb1, cb2, cb3, cb4, cb5,cb6,cb7,cb8,cb9,cb10,cb11,cb12,cb13,cb14,cb15
...
# 위젯 설정
    ...
    # 좌석 등급
    cb1 = QCheckBox("1루 블루석", win)
    cb2 = QCheckBox("1루 레드석", win)
    cb3 = QCheckBox("1루 네이비석", win)
    cb4 = QCheckBox("3루 블루석", win)
    cb5 = QCheckBox("3루 레드석", win)
    cb6 = QCheckBox("3루 네이비석", win)
    cb7 = QCheckBox("성민게이강퇴기원석", win)
    ...
    # 시작버튼
    b1 = QPushButton("복불복 티켓팅 시작 건투를 빕니다", win)
    b1.toggle()
...
# 입력한 정보 받아오기
    ...
    if cb6.isChecked() == True : cbCheck[5] = 1
    if cb7.isChecked() == True : cbCheck[6] = 1
...
# 매크로 시작
    ...
    # 인터파크 화면 띄우기
        # 드라이브 객체 생성
        url = './chromedriver.exe'  # 드라이브가 있는 경로
    ...
    # 예매하기_2
        ...
        notices = driver.find_element_by_css_selector('.schedule>li:nth-child(2)>.btn_sale>a')
        notices.click()
        # 활동로그
        log("예매창 띄우기")
    ...
    # 다양한 경우에 대한 빈 좌석 칮가
        ...
        blue = driver.find_element_by_xpath("//div[@class='list']/a[@sgn='"+tmp+"']")
        blue.click()
                ...
                for i in range(0, len(bs4.findAll('area')) + 1) :
                    ...
                    try:
                        ...
                        for i in range(0, len(seatList)):
                            ...
                            alert = text.switch_to_alert()
                            if (text.find("지정석 R석") != -1) & (cbCheck[0] == 1):
                                seatch = True
                                break
                            if (text.find("지정석 S석") != -1) & (cbCheck[1] == 1):
                                seatch = True
                                break
